import com.vin.java21nf.assignment.*;

import java.util.*;

void main() {
    System.out.println("--------SequencedCollection<LecturerRecord>---------");
    seqColl();
    System.out.println("--------SequencedSet<LecturerRecord>---------");
    seqSet();
    System.out.println("--------SequencedMap<LecturerRecord>---------");
    seqMap();
...
void recordPatterns(Object obj) {
    ...
    System.out.println(
            switch(obj){
                case LecturerRecord(String name, int age, Faculty faculty, Department dep)
                    when age >= 64 -> {
                    String output = """
                            Name: %s,
                            Age: %s,
                            Faculty: %s,
                            Department: %s
                            """.formatted(age,name,faculty,dep);
                    yield output;
                }
                case null, default -> "";
            }
